...
VERSIÓN 3: Usando EXACTAMENTE la misma estructura que cargar_estadisticas_agregadas.py
...
Comando: python backend/data_processing/02_unir_datasets_v3.py
"""
...
# Agregar backend al path (IGUAL que cargar_estadisticas_agregadas.py)
...
def extraer_datos_tabla(engine, tabla_nombre, tipo_operacion, limit=None):
    """
...
def main():
    """Función principal."""
    print("=" * 80)
    print("UNIFICACIÓN DE DATASETS - FASE 1 (V3)")
    print(f"Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("=" * 80)
    print()

    try:
        # Conectar a PostgreSQL (IGUAL que cargar_estadisticas_agregadas.py)
        ...
        print("2. Extrayendo datos de tablas...")
        df_inscripciones = extraer_datos_tabla(engine, 'datos_gob_inscripciones', 'inscripcion', LIMIT)
        df_transferencias = extraer_datos_tabla(engine, 'datos_gob_transferencias', 'transferencia', LIMIT)
        df_prendas = extraer_datos_tabla(engine, 'datos_gob_prendas', 'prenda', LIMIT)
