...
# -*- coding: utf-8 -*-
"""
Script para descargar datos históricos del BCRA.
...
- Tipo de cambio (USD oficial e informal)
- Tasas de interés (BADLAR, LELIQ)
- Reservas internacionales
- Base monetaria

Ejecutar desde: mercado_automotor/
Comando: python backend/data_processing/04_obtener_datos_bcra.py
"""
...
def descargar_datos_bcra(fecha_desde='2019-01-01', fecha_hasta=None):
    ...
    print(f"\n📋 Variables a descargar: {len(variables)}")
...
def validar_datos(df):
    ...
    for col in nulos.index:
        if nulos[col] > 0 and col not in ['anio', 'mes', 'trimestre']:
            print(f"   - {col:35} : {nulos[col]:5,} ({nulos_pct[col]:5.2f}%)")

    # 4. Estadísticas de variables clave
    print(f"\n📈 ESTADÍSTICAS DE VARIABLES CLAVE:")

    variables_clave = ['inflacion_mensual_oficial', 'usd', 'tasa_badlar', 'reservas']
...
def main():
    """Función principal."""
    print("="*80)
    print("OBTENCIÓN DE DATOS MACROECONÓMICOS - BCRA")
    print(f"Fecha: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    print("="*80)
