...
from backend.scrapers.validators import ProduccionData, validate_data
...
class AdefaScraper(BaseScraper):
    ...
    def _save_to_database(self, data: List[Dict[str, Any]]) -> int:
        ...
        Valida los datos con Pydantic antes de guardar.
        """
        if not data:
            return 0

        # Validar datos con Pydantic
        logger.info(f"[{self.name}] Validando {len(data)} registros...")
        valid_data, invalid_data = validate_data(data, ProduccionData)

        if invalid_data:
            logger.warning(
                f"[{self.name}] ⚠️ {len(invalid_data)} registros inválidos descartados"
            )
            # Log primeros errores para debugging
            for error_record in invalid_data[:3]:
                logger.debug(f"  Error: {error_record['error']}")

        if not valid_data:
            logger.error(f"[{self.name}] ✗ No hay datos válidos para guardar")
            return 0
        ...
        logger.info(f"[{self.name}] ✓ Guardados {saved_count} registros en BD ({len(valid_data)} validados)")
        return saved_count
